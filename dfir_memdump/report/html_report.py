...
{_render_encryption_keys(report.encryption_keys)}
...
def _render_finding_card(f, i: int) -> str:
...
def _render_findings_section(findings) -> str:
    """Render the Intelligence Findings section with a summary grid + grouped cards."""
    if not findings:
        return """
<section class="section">
  <h2>Intelligence Findings <span class="count-badge">0</span></h2>
  <p class="dim">No findings generated.</p>
</section>"""
    ...
    for sev in sev_order:
        ...
        icon = _SEV_ICON.get(sev, "⚪")
        sev_boxes += (
            f'<div class="findings-sev-box" style="border-color:{color};background:{bg}20">'
            f'<div class="findings-sev-count" style="color:{color}">{n}</div>'
            f'<div class="findings-sev-label">{icon} {sev}</div>'
            f'</div>'
        )

    cat_pills = " ".join(
        f'<span class="findings-cat-pill">{cat} <strong>{cnt}</strong></span>'
    ...
    for sev in sev_order:
        ...
        for f in grp:
            global_idx += 1
            cards += _render_finding_card(f, global_idx)

        # CRITICAL always expanded; others collapsed
        open_attr = "open" if sev == "CRITICAL" else ""
        groups_html += f"""
<details class="findings-group" {open_attr}>
  <summary class="findings-group-summary" style="border-left-color:{color}">
    <span class="sev-badge" style="color:{color};background:{bg}20">{icon} {sev}</span>
    <span class="findings-group-count">{len(grp)} finding{"s" if len(grp) != 1 else ""}</span>
    <span class="findings-group-titles dim">{" &nbsp;·&nbsp; ".join(_e(f.title[:60]) for f in grp[:3])}{"&nbsp;…" if len(grp) > 3 else ""}</span>
  </summary>
  <div class="findings-group-body">{cards}</div>
</details>"""
...
<section class="section">
  ...
  <details class="about"><summary>ℹ What is this?</summary>
    <p>Findings are generated by intelligence modules that analyse the raw Volatility3 output.
    CRITICAL findings are expanded by default — expand other severity groups as needed.
    Each finding includes the triggering evidence, MITRE ATT&amp;CK mapping, and click-to-copy IOCs.</p>
  </details>
  <div class="findings-summary">
    <div class="findings-sev-grid">{sev_boxes}</div>
    <div class="findings-cat-row">{cat_pills}</div>
  </div>
  <div class="findings-list">{groups_html}</div>
</section>"""
...
def _render_iocs(iocs) -> str:
    ...
    # Colour map by IOC type
    _IOC_COLORS = {
        "ip": "#ff4d4d", "hash_sha256": "#ff9933", "hash_md5": "#ff9933",
        "mutex": "#a371f7", "process_name": "#f0c040", "cmdline": "#f0c040",
        "filepath": "#58a6ff", "domain": "#ff4d4d", "url": "#ff4d4d",
        "aes_key": "#4caf50", "bitlocker_fvek": "#4caf50",
        "regkey": "#8b949e",
    }
    summary_boxes = ""
    for ioc_type, count in sorted(type_counts.items(), key=lambda x: -x[1]):
        color = _IOC_COLORS.get(ioc_type, "#8b949e")
        summary_boxes += (
            f'<div class="ioc-summary-box" style="border-color:{color}">'
            f'<div class="ioc-summary-count" style="color:{color}">{count}</div>'
            f'<div class="ioc-summary-label">{_e(ioc_type)}</div>'
            f'</div>'
        )
    ...
    rows = ""
    for ioc in iocs[:200]:
        color = _IOC_COLORS.get(ioc.type, "#8b949e")
        rows += (
            f'<tr><td><span class="ioc-type" style="color:{color}">{_e(ioc.type)}</span></td>'
    ...
    truncated = (
        f'<tr><td colspan="4" class="dim">… {len(iocs) - 200} more IOCs (see JSON report)</td></tr>'
        if len(iocs) > 200 else ""
    )
...
<section class="section">
  ...
  <details class="about"><summary>ℹ What is this?</summary>
    <p>Indicators of Compromise extracted from all findings. Click any value to copy it
    to your clipboard for import into a SIEM, threat intel platform, or firewall blocklist.</p>
  </details>
  <div class="ioc-summary-grid">{summary_boxes}</div>
  <details class="ioc-full-table">
    <summary>📋 Full IOC list ({len(iocs)} total)</summary>
...
<section class="section enc-keys-section">
  ...
  <p class="dim" style="padding:12px 0">
    ...
    for key in keys:
        dislocker_html = ""
        if key.dislocker_cmd and key.key_hex:
            dislocker_html = (
                f'<pre class="evidence-pre" style="margin-top:6px">{_e(key.dislocker_cmd)}</pre>'
            )
        key_display = (
            f'<code class="ioc-chip enc-key-hex" onclick="copyText(this)">{_e(key.key_hex)}</code>'
            if key.key_hex else '<span class="dim">— see notes —</span>'
        )
        source_badge = (
            f'<span class="cat-badge" style="color:#4caf50">{_e(key.source)}</span>'
        )
        pid_str = f"PID {key.pid} ({_e(key.process_name)})" if key.pid else ""
        rows += f"""
<div class="enc-key-card">
  <div class="enc-key-header">
    <span class="enc-key-type">🔑 {_e(key.key_type)}</span>
    <span class="enc-key-algo">{_e(key.algorithm)}</span>
    {source_badge}
    {f'<span class="dim enc-key-pid">{pid_str}</span>' if pid_str else ""}
  </div>
  <div class="enc-key-body">
    <div class="finding-meta-row">
      <span class="meta-label">Key (hex)</span>
      {key_display}
    </div>
    {f'<div class="finding-meta-row"><span class="meta-label">Mount cmd</span>{dislocker_html}</div>' if dislocker_html else ""}
    {f'<div class="finding-meta-row"><span class="meta-label">Notes</span><span class="dim">{_e(key.notes)}</span></div>' if key.notes else ""}
    {f'<div class="finding-meta-row"><span class="meta-label">Offset</span><code>{_e(key.file_offset)}</code></div>' if key.file_offset else ""}
  </div>
</div>"""

    return f"""
<section class="section enc-keys-section">
  <h2>🔑 Encryption Key Artifacts <span class="count-badge">{len(keys)}</span></h2>
  <details class="about"><summary>ℹ What is this?</summary>
    <p>Encryption key material recovered from the memory image. Sources:</p>
    <ul>
      <li><strong>windows.bitlocker.Bitlocker</strong> — Full Volume Encryption Keys (FVEKs) extracted by Volatility3.
          Pass directly to <code>dislocker</code> or <code>bdemount</code> to mount the volume.</li>
      <li><strong>aeskeyfind</strong> — AES key schedules detected by statistical analysis of the raw image.
          May match BitLocker, VeraCrypt, TrueCrypt, or other AES-based ciphers.</li>
      <li><strong>bulk_extractor</strong> — AES key material found by the bulk_extractor -e aes scanner.</li>
      <li><strong>process-presence</strong> — VeraCrypt/TrueCrypt was running at collection time;
          key material is likely in its VAD. Use aeskeyfind/bulk_extractor to recover candidates.</li>
    </ul>
    <p><strong>Chain of custody:</strong> All key recovery was performed against the forensic memory copy.
    Document tool versions and commands used — these are discoverable and must appear in your notes.</p>
  </details>
  {rows}
</section>"""
...
_CSS = """
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Findings summary ── */
  .findings-summary { margin-bottom: 14px; }
  .findings-sev-grid { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 10px; }
  .findings-sev-box { border: 1px solid; border-radius: 8px; padding: 10px 16px; min-width: 100px; text-align: center; }
  .findings-sev-count { font-size: 26px; font-weight: 700; }
  .findings-sev-label { font-size: 11px; margin-top: 2px; }
  .findings-cat-row { display: flex; flex-wrap: wrap; gap: 6px; }
  .findings-cat-pill { background: var(--bg3); border: 1px solid var(--border); border-radius: 12px; padding: 2px 10px; font-size: 11px; color: var(--text-dim); }
  .findings-cat-pill strong { color: var(--text); }
  /* Findings groups */
  details.findings-group { border: 1px solid var(--border); border-radius: 8px; margin-bottom: 8px; overflow: hidden; }
  .findings-group-summary { display: flex; align-items: center; gap: 8px; padding: 10px 14px; cursor: pointer; list-style: none; border-left: 3px solid transparent; user-select: none; }
  .findings-group-summary::-webkit-details-marker { display: none; }
  .findings-group-summary:hover { background: var(--bg3); }
  .findings-group-count { font-size: 12px; color: var(--text-dim); white-space: nowrap; }
  .findings-group-titles { font-size: 11px; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .findings-group-body { padding: 8px 10px; background: var(--bg); border-top: 1px solid var(--border); }

  /* ── IOC summary ── */
  .ioc-summary-grid { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 12px; }
  .ioc-summary-box { border: 1px solid; border-radius: 8px; padding: 8px 14px; min-width: 90px; text-align: center; background: var(--bg2); }
  .ioc-summary-count { font-size: 22px; font-weight: 700; }
  .ioc-summary-label { font-size: 10px; color: var(--text-dim); margin-top: 2px; text-transform: uppercase; letter-spacing: 0.04em; }
  details.ioc-full-table > summary { cursor: pointer; color: var(--accent); font-size: 13px; padding: 6px 0; }

  /* ── Encryption key artifacts ── */
  .enc-keys-section h2 { color: #4caf50; border-bottom-color: #4caf50; }
  .enc-key-card { background: var(--bg2); border: 1px solid #4caf50; border-radius: 8px; margin-bottom: 10px; overflow: hidden; }
  .enc-key-header { display: flex; align-items: center; gap: 10px; padding: 10px 14px; background: #0a1f0a; flex-wrap: wrap; }
  .enc-key-type { font-size: 14px; font-weight: 600; color: #4caf50; }
  .enc-key-algo { font-size: 12px; color: var(--text-dim); background: var(--bg3); padding: 1px 8px; border-radius: 4px; }
  .enc-key-pid { font-size: 11px; }
  .enc-key-body { padding: 12px 16px; }
  .enc-key-hex { font-size: 11px; word-break: break-all; max-width: 100%; }
